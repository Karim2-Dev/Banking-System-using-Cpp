...
vector<string> MySplit(string Line, string delim) {
  ...
  while ((Position = Line.find(delim)) != string::npos) {
    ...
    if (sWord != "") { vMySplit.push_back(sWord); }
...
vector<stClient> saveClientsDataToFile(
  string FileName, vector<stClient>& vClients) {
  fstream MyFile;
...
bool FindClient(vector<stClient>& vClients, string AccNum) {
  for (stClient& stClient : vClients) {
    if (stClient.AccNum == AccNum) { return true; }
  }
  return false;
}
...
bool MarkClientForDeleteByAccountNumber(
  string AccountNumber, vector<stClient>& Clients) {
...
stClient ReadNewClient(vector<stClient>& Clients) {
  ...
  while (FindClient(Clients, AccData.AccNum)) {
    printf("Client with [%s] already exists, Enter another Account Number? ",
      AccData.AccNum.c_str());
    cin >> AccData.AccNum;
  }
...
void PrintHeaderDatastClients(vector<stClient>& stClientsData) {
  cout << setw(75) << "stClient List (" << stClientsData.size()
       << ") stClients(s)";
...
int GetTotalBalances(vector<stClient>& Clients) {
  int Total = 0;
  for (stClient& Client : Clients) { Total += Client.AccountBalance; }
  return Total;
};
...
short ReadPermission() {
  ...
  if (tolower(IsAccess) == 'y') { flags |= enPermission::enClientListAccess; }

  cout << "\nAdd New Client? y/n ";

  cin >> IsAccess;

  if (tolower(IsAccess) == 'y') { flags |= enPermission::enAddNewClientAccess; }

  cout << "\nDelete Client? y/n ";
  cin >> IsAccess;

  if (tolower(IsAccess) == 'y') { flags |= enPermission::enDeleteClientAccess; }

  cout << "\nUpdate Client? y/n ";
  cin >> IsAccess;

  if (tolower(IsAccess) == 'y') { flags |= enPermission::enUpdateClientAccess; }

  cout << "\nFind Client? y/n ";
  cin >> IsAccess;

  if (tolower(IsAccess) == 'y') { flags |= enPermission::enFindClientAccess; }

  cout << "\nTransactions? y/n ";
  cin >> IsAccess;

  if (tolower(IsAccess) == 'y') { flags |= enPermission::enTransactionsAccess; }

  cout << "\nManage Users? y/n ";
  cin >> IsAccess;
  if (tolower(IsAccess) == 'y') { flags |= enPermission::enManageUsersAccess; }

  if (flags == 127) { flags = -1; }
...
stUser ReadNewUser() {
  ...
  while (isFindUser(User.UserName, vUsers)) {
    printf("User with [%s] already exists, Enter another Username? ",
      User.UserName.c_str());
    cin >> User.UserName;
  }
  ...
  if (MyFile.is_open()) {
    ...
    for (stUser& User : vUsers) {
      if (User.UserName == UserName) {
        ...
        if (User.UserName == CurrentUser.UserName) { CurrentUser = User; }
      }
      MyFile << ConvertRecordToLine(User) << endl;
    }
  }
...
bool DeleteClientByAccountNumber(
  string AccountNumber, vector<stClient>& vClients, string FileName) {
  stClient Client;
  char Answer = 'n';
...
bool EditClientByAccountNumber(
  string AccountNumber, vector<stClient>& vClients, string FileName) {
  stClient Client;
  char Answer = 'N';
  if (FindClient(vClients, AccountNumber, Client)) {
...
bool DepositClient(vector<stClient>& vClients, string FileName) {
  ...
  do {
    ...
    if (isFindClient) {
      ...
      if (toupper(Answer) == 'Y') {
        for (stClient& C : vClients) {
          if (C.AccNum == AccountNumber) { C.AccountBalance += DepositAmount; }
        }
        saveClientsDataToFile(FileName, vClients);
      }
    } else {
  ...
  do {
    ...
    if (isFindClient) {
      ...
      while (WithdrawAmount > Client.AccountBalance) {
        printf("\nAmount Exceeds the balance, you can withdraw up to: %.*f", 3,
...
void UpdateUserScreen() {
  ...
  if (FindUser(UserName, vUsers, User)) {
    ...
    if (tolower(Answer) == 'y') {
      ...
      printf(
        "Client with Account Number (%s) is Not Found! ", UserName.c_str());
    }
...
void GetMainMenuOption(bool& IsLogIn) {
  ...
  switch (Page) {
    ...
    case enMainMenuOptions::enDeleteClient:
      ...
      if (IsAccess(
            CurrentUser.Permission, enPermission::enDeleteClientAccess)) {
    ...
    case enMainMenuOptions::enUpdateClient:
      ...
      if (IsAccess(
            CurrentUser.Permission, enPermission::enUpdateClientAccess)) {
    ...
    case enMainMenuOptions::enTransactionClient:
      ...
      if (IsAccess(
            CurrentUser.Permission, enPermission::enTransactionsAccess)) {
...
void GetTransactionOption() {
  enTransacitonOptions Page = (enTransacitonOptions)MyInputLib::ReadNumber(
    "\nChoose what do you want to do [1,6]");
  switch (Page) {
    case enTransacitonOptions::Deposit: DepositPage(); break;
    case enTransacitonOptions::Withdraw: WithdrawPage(); break;
    case enTransacitonOptions::TotalBalances: TotalBlancesPage(); break;
...
void GetManageUsersOption() {
  enManageUsersOptions Page = (enManageUsersOptions)MyInputLib::ReadNumber(
    "\nChoose what do you want to do [1,6]");
  switch (Page) {
    case enManageUsersOptions::enShowUsersList: UsersListScreen(); break;
    case enManageUsersOptions::enAddNewUser: AddNewUserScreen(); break;
    case enManageUsersOptions::enDeleteUser: DeleteUserScreen(); break;
    case enManageUsersOptions::enUpdateUser: UpdateUserScreen(); break;
    case enManageUsersOptions::enFindUser: FindUserScreen(); break;
...
void LoginScreen() {
  ...
  bool IsLogIn = true;
  while (IsLogIn) { PrintMainMenuScreeen(IsLogIn); }
}
}  // namespace BankSystem